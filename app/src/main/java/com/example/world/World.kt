...
class World {
    ...
    fun getTerrainHeight(x: Int, z: Int): Int {
        ...
        val baseNoise = SimplexNoise.noise2D(nx, nz) * 12f
        val detailNoise = SimplexNoise.noise2D(nx * 4.0f, nz * 4.0f) * 3f
    ...
    fun getBiomeAt(x: Int, z: Int): Byte {
        ...
        val moisture = SimplexNoise.noise2D(x.toFloat() * 0.01f + 500f, z.toFloat() * 0.01f + 500f)
    ...
    fun isCave(x: Int, y: Int, z: Int): Boolean {
        ...
        val caveNoise = SimplexNoise.noise3D(x.toFloat() * 0.07f, y.toFloat() * 0.07f, z.toFloat() * 0.07f)
    ...
    fun generateInitialWorld() {
        ...
        // 5x5 chunks (-2 to 2) - robust 80x80 blocks landscape
        for (cx in -2..2) {
            for (cz in -2..2) {
